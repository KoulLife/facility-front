...
const ImprovementList = () => {
    ...
    const fetchItems = async (facilityTypeId) => {
        ...
        try {
            ...
            if (response.data && Array.isArray(response.data)) {
                ...
                const savedOrder = JSON.parse(localStorage.getItem('improvementListOrder'));
                if (savedOrder) {
                ...
                localStorage.setItem('improvementListItems', JSON.stringify(fetchedItems));
            }
        } catch (error) {
    ...
    const onDragEnd = (result) => {
        ...
        setItems(newList);
        localStorage.setItem('improvementListOrder', JSON.stringify(newList.map(item => item.id)));
        localStorage.setItem('improvementListItems', JSON.stringify(newList));
    };

    const handleItemClick = (item) => {
        navigate(`/checker/improvement-info/${item.id}`, {
            state: {
                facilityId: item.id,
...
export default validationAuth(ImprovementList);